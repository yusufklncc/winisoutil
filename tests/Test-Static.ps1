...
foreach ($language in @('tr', 'en')) {
    ...
    $orphanDescriptionKeys = @($global:langStrings.Keys | Where-Object {
        $_ -match '^(tweak_|comp_|feature_).+_desc$' -and $_ -notin $expectedDescriptionKeys
    })
...
Assert-True -Condition ($mainScript -match 'SchemaVersion\s*=\s*2') -Message 'WinISOUtil exports must use configuration schema version 2.'
...
Assert-True -Condition ($mainScript -match '\$selector -cnotmatch') -Message 'Provisioned app selector validation must use a case-sensitive ASCII allow-list.'
